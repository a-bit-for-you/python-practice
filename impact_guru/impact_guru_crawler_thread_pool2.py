    ...
    for i in re.split('<a\s*class="nav-link\s*category-nav-link', response)[1:]:
        ...
        while True:
            ...
            for j in page.split('box-shadow">')[1:]:
                ...
                fundraiser_urls.append(fundraiser_url)
                
                
def insert_data(fundraiser_url):    
...
with concurrent.futures.ThreadPoolExecutor() as executor:
    executor.submit(index_urls)
    
print(len(fundraiser_urls))
print(f'Indexing finished in {time.time() - start_time} seconds.\n')  

# We can use a with statement to ensure threads are cleaned up promptly
with concurrent.futures.ThreadPoolExecutor() as executor:
    # Start the load operations and mark each future with its URL
    future_to_url = {executor.submit(insert_data, fundraiser_url): fundraiser_url for fundraiser_url in fundraiser_urls}
    for future in concurrent.futures.as_completed(future_to_url):
        fundraiser_url = future_to_url[future]
        try:
            data = future.result()
            # inserting data
            fundraisers_col.insert_one(data)
        except Exception as exc:
            print('%r generated an exception: %s' % (fundraiser_url, exc))
        else:
            print('%r page is %d bytes' % (fundraiser_url, len(data)))
